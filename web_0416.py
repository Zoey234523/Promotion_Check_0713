...
def validate_data(df):
    required_columns = ['当前职级', '本次绩效', '上次绩效', '当前职级在职时长（月）', '是否带团队']
...
if uploaded_file is not None:
    ...
    2. **本次绩效**: 当前评估周期的绩效评级 (M-, M, M+, E, O)
    3. **上次绩效**: 上一评估周期的绩效评级 (M-, M, M+, E, O)
    4. **当前职级在职时长（月）**: 员工在当前职位上的工作月数 (例如 6 表示6个月)
    5. **是否带团队**: 员工是否担任领导角色 (Y 或 N)
    
    示例:
    
    | 员工ID | 姓名 | 当前职级 | 本次绩效 | 上次绩效 | 当前职级在职时长（月） | 是否带团队 |
    |--------|------|----------|----------|----------|------|------------|
    | 001    | 张三 | I2-1     | M+       | M        | 6  | N          |
    | 002    | 李四 | E1-1     | E        | M+       | 24  | Y          |
    """)

# Footer
st.markdown("---")
st.markdown("© 2025 员工晋升资格评估工具")
