def promotion_check(cur_position, cur_performance, last_performance, work_month, if_leader):
    performance = [['M','M+','E','O'],['M-']]
    ...
    # 晋升绩效条件
    curmm_lastm = cur_performance in performance[0][1:]   #本次M+，上次M
    both_mm = cur_performance in performance[0][1:] and last_performance in performance[0][1:]  #两次M+
    mm_plus_e = (cur_performance in performance[0][1:] and last_performance in performance[0][2:]) or \
                    (cur_performance in performance[0][2:] and last_performance in performance[0][1:])     #一次M+一次E
    both_e = cur_performance in performance[0][2:] and last_performance in performance[0][2:]   #两次E
    curo_lastmm = cur_performance == 'O' and last_performance in performance[0][1:]  #本O上E
    e_plus_o = (cur_performance in performance[0][2:] and last_performance in performance == 'O') or \
                    (cur_performance == 'O' and last_performance in performance[0][2:])     #一次O一次E
    curo_lastmm = cur_performance == 'O' and last_performance in performance[0][1:]  #本O上M+
    mm_plus_o = (cur_performance in performance[0][1:] and last_performance == 'O') or \
                    (cur_performance == 'O' and last_performance in performance[0][1:])     #一次M+一次O(管理升专业，可能有问题)
    mminus_inclu = cur_performance=='M-' or last_performance=='M-' # 包含一次 M-
    ...
    if if_leader == 'N':
        ...
        if cur_position in prof_rank[:3]:
            ...
            if mm_plus_o and work_month >= 12:
    ...
    if if_leader == 'Y':
        if cur_position == 'E1-1':
            if (both_e or curo_lastmm) and work_month >= 18:
